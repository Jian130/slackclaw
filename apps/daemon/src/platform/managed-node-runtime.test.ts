...
async function createFakeNodeArchive(
  root: string,
  version: string,
  options?: { compression?: "gzip" | "xz" }
): Promise<string> {
  ...
  const archivePath = join(root, `${distName}.${compression === "xz" ? "tar.xz" : "tar.gz"}`);
...
`
  ...
  await execFile("/usr/bin/tar", [compression === "xz" ? "-cJf" : "-czf", archivePath, "-C", root, distName]);
...
test("ensureManagedNodeNpmInvocation installs from the official tar.xz Node archive shape", async () => {
  const tempDir = await mkdtemp(join(tmpdir(), "chillclaw-managed-node-xz-test-"));
  const dataDir = join(tempDir, "data");
  const version = "99.1.0";
  const archivePath = await createFakeNodeArchive(tempDir, version, { compression: "xz" });
  const originalDataDir = process.env.CHILLCLAW_DATA_DIR;
  const originalNodeVersion = process.env.CHILLCLAW_MANAGED_NODE_VERSION;
  const originalNodeUrl = process.env.CHILLCLAW_MANAGED_NODE_DIST_URL;
  const restorePlatform = mockProcessPlatform("darwin");
  ...
  try {
    ...
    assert.equal(invocation.command, getManagedNodeNpmBinPath());
    assert.equal(invocation.argsPrefix.length, 0);
    await execFile(invocation.command, [...invocation.argsPrefix, "--version"]);
  } finally {
    restorePlatform();
    if (originalDataDir === undefined) {
      delete process.env.CHILLCLAW_DATA_DIR;
