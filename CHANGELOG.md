...
- fixed onboarding completion gateway token mismatches by replacing unusable OpenClaw placeholder gateway tokens during ChillClaw baseline normalization and reapplying the safe local gateway baseline before restart verification
- stopped repeated no-op daemon state writes from channel/session polling by skipping persistence when a normalized state update does not change stored state
